...
from engine.utils import index_to_square, square_to_index, is_white
    ...
    if move.is_castling and piece.upper() == 'K':
        ...
        if piece_letter:
            ...
            if same_dests:
                ...
                need_file = any(m.from_sq % 8 == from_file for m in same_dests)
                need_rank = any(m.from_sq // 8 == from_rank for m in same_dests)
                # If both share file or rank, include both
                if need_file and need_rank:
                    disamb = index_to_square(move.from_sq)[0] + index_to_square(move.from_sq)[1]
                elif need_file:
                    disamb = index_to_square(move.from_sq)[1]
                elif need_rank:
                    disamb = index_to_square(move.from_sq)[0]
                else:
                    # Only one shares either rank or file; include whichever distinguishes
                    disamb = index_to_square(move.from_sq)[0]
        # Destination
