...
- Secret code generation from a given set of colors.
...
- Partial matches count colors present in both guess and secret but not already counted
  as exact matches. This uses min count over unmatched elements per color.
'''
...
class MastermindGame:
    ...
    Attributes:
        ...
        colors: List of available color keys (strings).
        max_attempts: Maximum number of attempts allowed.
        allow_duplicates: Whether the secret code can contain duplicate colors.
    """
    ...
    def __init__(
        ...
        if not allow_duplicates and code_length > len(colors):
            raise ValueError("code_length cannot exceed number of colors when duplicates are not allowed")
    ...
    def evaluate_guess(self, guess: List[str]) -> Tuple[int, int]:
        ...
        - partial: correct color but in the wrong position (not counting exact matches).
        """
        if self.is_over():
        ...
        if any(g not in self.colors for g in guess):
            raise ValueError("Guess contains invalid colors.")
        ...
        # Partial matches are the sum of min counts per color over unmatched items
