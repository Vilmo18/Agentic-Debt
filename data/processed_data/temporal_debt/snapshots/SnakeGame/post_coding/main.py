...
initializes Pygame and launches the game loop.
...
def choose_difficulty(screen):
    ...
    button_height = 54
    spacing = 16
    start_y = HEIGHT // 2 - (button_height * 2 + spacing * 3) // 2
    ...
    while True:
        ...
        for btn in buttons:
            btn.draw(screen if not btn.is_hovered(mouse_pos) else screen)

        # Hints
        draw_text(
            screen,
            "Choose difficulty. ESC to quit.",
            font_hint,
            (200, 200, 200),
            (WIDTH // 2, HEIGHT - 40),
        )
