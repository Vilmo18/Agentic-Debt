    ...
    def _load(self) -> None:
        ...
        puzzles_seed = [
            ...
            StrandPuzzle(
                ...
                phrases=["machine learning", "artificial intelligence", "data science", "neural network", "big data", "deep learning"],
            ),
        ]
...
def verify_merge(puzzle: StrandPuzzle, selected_ids: List[str]) -> Dict:
    ...
    # Check if selected are in strictly increasing order
    ...
    if increasing_order:
        # Contiguity already implies no gaps due to +1 check
        start_idx = selected_seg_indices[0]
        end_idx = selected_seg_indices[-1]

        # Determine whether it's the full phrase
...
def _segment_from_any(puzzle: StrandPuzzle, seg_id: str) -> Optional[Segment]:
    """
    Attempts to reconstruct a Segment from seg_id if it no longer exists in puzzle.segments
    (e.g., already used). This is safe for verification logic across full session.
    """
    try:
        ...
        return None
    # Recreate text by taking phrase normalized letters and splitting with the same algorithm/seed
    # Build a replicated segmentation for the phrase only
    rng = random.Random(puzzle.id)
    letters = normalize(puzzle.phrases[p_idx])
    parts = StrandPuzzle._split_into_segments(letters, rng)
    # Re-run splits for all earlier phrases to align RNG? Our splitter is deterministic without randomness across phrases
    # Actually, the splitter itself uses rng, but not its state across phrases; only per-call randomness.
    # We called it once above, so this is correct.
    if s_idx < 0 or s_idx >= len(parts):
        return None
    return Segment(id=seg_id, text=parts[s_idx], phrase_index=p_idx, seg_index=s_idx)
