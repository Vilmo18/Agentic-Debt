...
class MinesweeperGame:
    ...
    def elapsed_time(self) -> float:
        ...
        if self.state in ("won", "lost"):
            # Freeze time at end
            return max(0.0, self._end_time() - self.start_time)
        return time.time() - self.start_time

    def _end_time(self) -> float:
        """
        Determine the end timestamp for timing purposes.
        """
        # For simplicity, when the game ends, consider end time as now.
        # In a richer implementation, store the exact moment when state changed.
