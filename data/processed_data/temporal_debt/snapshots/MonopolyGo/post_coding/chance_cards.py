...
class ChanceDeck:
    ...
    def _build_cards(self):
        ...
        def move_back_3(game, player):
            ...
            new_pos = (player.position + steps) % len(game.board.tiles)
            # No GO salary when moving backwards
            player.position = new_pos
            msgs = [f"Chance: Move back 3 spaces."]
            msgs += game.process_current_tile(player)
            return msgs
    ...
    def draw(self, game, player) -> List[str]:
        ...
        while attempts < len(self.cards):
            ...
            if card.is_get_out_of_jail:
                # Hold: do not put back until returned
                self.cards.append(card)  # Keep in deck order, but mark as unavailable
            else:
                self.cards.append(card)
            return messages
