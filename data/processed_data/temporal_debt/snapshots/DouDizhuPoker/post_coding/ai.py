...
class SimpleAI:
    ...
    def _find_low_pair_sequence_start(self, hand: List[Card]) -> Optional[List[Card]]:
        ranks = sorted(set(r for r in Counter(c.rank for c in hand if 3 <= c.rank <= 14) if Counter(c.rank for c in hand)[r] >= 2))
    ...
    def _find_low_triple_sequence_start(self, hand: List[Card]) -> Optional[List[Card]]:
        ranks = sorted(set(r for r in Counter(c.rank for c in hand if 3 <= c.rank <= 14) if Counter(c.rank for c in hand)[r] >= 3))
