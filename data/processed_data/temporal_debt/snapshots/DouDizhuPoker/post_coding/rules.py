...
def identify_combination(cards: List[Card]) -> Optional[Combination]:
    ...
    if n % 4 == 0 and n >= 8:
        ...
        if run and len(run) == m:
            # ensure we can take exactly 3 from each, and have m singles (not using triple ranks ideally)
            # Count singles after removing triples
            leftover_counts = count_map.copy()
            for tr in run:
                leftover_counts[tr] -= 3
            singles = []
            for r, cnt in leftover_counts.items():
                singles.extend([r] * cnt)
            if len(singles) >= m:
    ...
    if n % 5 == 0 and n >= 10:
        ...
        if run and len(run) == m:
            ...
            for tr in run:
                leftover_counts[tr] -= 3
            # count pairs in leftover
            pair_units = sum(leftover_counts[r] // 2 for r in leftover_counts.keys())
            if pair_units >= m:
