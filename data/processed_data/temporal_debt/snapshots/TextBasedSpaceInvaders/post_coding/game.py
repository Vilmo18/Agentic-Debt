...
class AlienFleet:
    ...
    def update(self):
        ...
        # Move horizontally
        dx = int(self.speed) if abs(self.speed - int(self.speed)) < 1e-6 else self.speed
        for alien in self.aliens:
            # Use sub-pixel move by accumulating fractional part via rect.x float workaround:
            # Since rect.x is int, we approximate by rounding speed each frame.
            alien.rect.x += int(self.direction * round(self.speed))

    def reached_bottom(self, player_top: int) -> bool:
        if len(self.aliens) == 0:
            return False
        max_bottom = max(alien.rect.bottom for alien in self.aliens)
        return max_bottom >= player_top
...
class Game:
    ...
    def update(self):
        if self.state == "PLAYING":
            ...
            # Check bottom reach condition
            if self.fleet.reached_bottom(self.player.rect.top):
    ...
    def resolve_collisions(self):
        ...
        hits = pygame.sprite.groupcollide(self.aliens, self.bullets, dokilla=True, dokillb=True)
        if hits:
            destroyed = sum(len(bullets) for bullets in hits.values())
            # Score per alien destroyed (ignore multiple bullets per alien; count aliens)
        ...
        if now >= self.player_safe_until_ms:
            collided = pygame.sprite.spritecollide(self.player, self.aliens, dokill=True)
            if collided:
                self.lose_life()
    ...
    def lose_life(self):
        ...
        self.player.rect.bottom = settings.HEIGHT - (settings.PLAYER_Y_OFFSET - settings.PLAYER_HEIGHT // 2)
    ...
    def draw(self):
        ...
        # Draw stars background (simple parallax dots)
        random.seed(1)  # deterministic stars per frame
        for _ in range(80):
            x = random.randint(0, settings.WIDTH - 1)
            y = random.randint(0, settings.HEIGHT - 1)
            col = (40, 40, 40)
            self.screen.fill(col, rect=pygame.Rect(x, y, 2, 2))
